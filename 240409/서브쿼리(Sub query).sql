...
-- 1) IN 함수 : 목록 안에 값이 포함되었는지 확인 
SELECT * FROM emp WHERE deptno IN (10, 20, 30);   

-- 서브쿼리
SELECT empno FROM emp WHERE sal > 2000;		
-- 2000보다 급여를 많이 받는 6명(행) 조회 결과  
...
-- 3) ALL 함수 
-- (800, 2975, 3000, 1100, 3000) 여러 행의 결과 값 모두 보다 이상이어야 참
-- 3000(최댓값)보다 큰 경우만 참 
SELECT ename, sal FROM emp
...
-- 4) EXISTS : 주로 상호 연관 서브쿼리에서 유용하게 사용 
-- 상호연관 쿼리 : 메인 쿼리의 컬럼을 서브쿼리에서 사용하는 것 
...
-- 생성된 임시 테이블은 쿼리 실행 시점에만 존재하고 사라짐 => 쿼리문 안에서만 사용
-- FROM 절의 임시테이블은 약칭을 주어 사용 해야 한다. 
