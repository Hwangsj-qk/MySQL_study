...
1. 숫자형 데이터 타입
   ...
   - FOLAT, DOUBLE: 부동소수점(4byte, 8byte)

2. 문자열 데이터 타입
   - CHAR(N) : 고정 길이 문자열 ➡ N은 문자열의 길이, 문자열이 N보다 짧으면 공백으로 채워짐.
   - VARCHAR(N) : 가변 길이 문자열 ➡ N은 문자열의 길이, 실제 사용된 길이만큼 저장공간 차지. ▶ 최대값 기본: 225 바이트 MySQL 버전에 따라 최대 바이트가 다름 
   - TEXT : 긴 텍스트를 저장 최대 65,535 바이트
   - MEDIUMTEXT : 최대 15MB까지 저장
...
3. 날짜 시간 데이터타입
   ...
   - TIMESTAMP: 유닉스 타임스탬프 기반 날자 시간정보 저장 (1970-01-01 ➡ 32비트 정수)
4. 이진 데이터 타입
   - BINARY(N) : 고정길이 (이미지, 파일 등 저장에 사용)
   - VARBINARY(N) : 가변길이
...
- 일반적으로 사용자에게 최소한(필요한)의 권한만 부여하는 **최소 권한 원칙**을 따름  
