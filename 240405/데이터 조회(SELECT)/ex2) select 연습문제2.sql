...
SELECT CountryCode, Language FROM CountryLanguage WHERE IsOfficial = 'T' AND Language = 'French';
...
WHERE  NOT (IsOfficial = 'T' and Language = 'English') AND Percentage >= 50.0;
...
-- 문제 12: City 테이블에서 도시 이름(Name)이 'S'로 시작하고, 
--         인구(Population)가 1000000 이상인 도시의 이름(Name), 국가 코드(CountryCode), 인구(Population)를 조회하세요.
