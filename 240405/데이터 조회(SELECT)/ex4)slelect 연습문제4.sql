...
SELECT NAME, GNP FROM COUNTRY WHERE Continent = 'Asia'
ORDER BY GNP DESC
LIMIT 5;
...
-- 문제 5: Country 테이블에서 인구가 5천만 이상 1억 미만인 국가들을 
--        인구 순서대로 오름차순 정렬하여 국가의 이름(Name)과 인구(Population)를 조회하세요.
...
SELECT DISTINCT GovernmentForm, continent FROM Country ORDER BY continent;
-- 정렬기준은 선택된 컬러명에 존재해야 한다. 