...
/* 1. READ UNCOMMITTDD, 더티 리드 */
-- 고립 수준 변경하기
SET TRANSACTION isolation level read uncommitted;
START TRANSACTION;
-- 읽기(세션2에 쓰기 전) => 홍길동 나이 30
SELECT * FROM person;

-- 읽기(세션2에서 쓴 후) => 홍길동 나이 50
-- 세션 2는 커밋을 하지 않았는데 변경 내용이 반영되어 읽어짐 => READ UNCOMMITTED
-- 더티 리드(오손 읽기, dirty read)
SELECT * FROM person;

-- 읽기(세션2에서 트랜잭션 롤백 후) => 홍길동 나이 30
-- 세션2에서 작업한 데이터가 원상복귀
-- 무효화 되는 데이터를 읽은 것 -> 결과적으로 잘못된 결과를 읽게 됨 
SELECT * FROM person;
...
-- 읽기 : 세션 2 커밋 후 (홍길동 나이가 변경됨) => 그대로 30
