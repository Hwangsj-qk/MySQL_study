...
-- MySQL 기본 페이지 크기 확인하기 (기본값 16KB - 필요하면 변경 가능)
...
-- 현재 인덱스가 없는 상태
SHOW INDEX FROM clustertbl;		-- 제약조건(primary key 등)이 없으므로 인덱스가 없음
...
-- 클러스터형 인덱스의 루트 페이지 => (리프페이지 또는 인터널페이지)의 "참조 주소(=1000, 1001, 1002,...)를가지고 있음
-- 클러스터형 인덱스의 리프 페이지 => 실제 데이터가 저장되어 있음 
...
-- 하나의 인덱스 생성 (데이터 정렬X)
-- 보조 인덱스가 생성되고, 데이터는 조회 시 userId 컬럼 기준으로 정렬되어 있지 않음. 
-- 보조 인덱스의 리프 페이지에서는 인덱스 컬럼(userId) 기준으로 정렬되어 있음. 
-- 리프 페이지는 실제 데이터를 가지고 있는 것이 아니라,
...
-- 페이지 분할은 시스템에 많은 부하를 주게됨
INSERT INTO clustertbl VALUES ('FNT', '푸나타');
INSERT INTO CLUSTERTBL VALUES('KAI', '카아이');

-- 보조 인덱스에 데이터를 삽입하게 될 경우
-- 실제 데이터페이지의 빈 부분에 삽입되고
-- 인덱스 위치만 조정하게 됨 -> 클러스터형 인덱스보다 성능을 주는 부하가 적음
